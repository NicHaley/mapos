...
- **`apps/web`'s generated types are committed, so a fresh clone needs no codegen.** `cloudflare-env.d.ts` is checked in (`next-env.d.ts` is covered by the committed `apps/web/types/next-ambient.d.ts`). **Adding a binding or a secret means rerunning `pnpm --filter=@mapos/web cf-typegen` and committing the result** — wrangler reads secret *names* from `apps/web/.dev.vars`, so add new ones to `.dev.vars.example` too. CI runs `cf-typegen --check` and fails if either was skipped.
