...
- R2 bucket name. Defaults to `mapos-updates`. Override with `MAPOS_R2_BUCKET=<name>` when running the release script.
...
3. **Build the macOS artifacts**:
   ...
   ```
   Produces, in `apps/dashboard/dist/`:
   - `MapOS-<version>.dmg` + `.blockmap`
   - `MapOS-<version>-arm64-mac.zip` + `.blockmap`
   - `latest-mac.yml` (the manifest electron-updater reads)
...
6. **Smoke test** with an older installed build:
...
7. **Push the tag** so the release is recorded:
   ```
...
- `scripts/publish-release.sh` — the upload script. Reads version from `package.json`.
