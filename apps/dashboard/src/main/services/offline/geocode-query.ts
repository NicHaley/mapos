 ...
 * this queries every installed pack's DB and merges the results — forward search
 * ranks across all packs, reverse search only hits packs whose bbox covers the
 * point.
 *
 ...
 *
 * **This module runs inside the geocode worker thread, never on the main thread.**
 * `better-sqlite3` is synchronous, and ranking a broad prefix match ("br"* matches
 * ~776k rows in the us-new-york pack) costs hundreds of milliseconds to seconds —
 * on the main thread that's a frozen app, beach ball and all. `geocoding.ts` is the
 * main-thread client that talks to this over a message port; keep the two apart.
 */
...
export function forwardQuery(req: GeocodeForwardRequest, ep: Endpoint): GeocodeResult[] {
  ...
  if (match) {
    params.match = match;
    // Exact-name boost: the normalised whole query. When it equals a feature's name, that
    // feature gets a large score bonus so the canonical place ("Berlin") beats partial-token
    // matches ("Berliner Straße") — including across packs, where raw bm25 scores aren't
    // comparable.
    params.exact = exactPhrase(req.query ?? "");
  }
...
export function reverseQuery(req: GeocodeReverseRequest, ep: Endpoint): GeocodeResult[] {
...
/**
 * Wire protocol between the main-thread client (`geocoding.ts`) and the worker
 * (`geocode-worker.ts`). Every payload is structured-clonable plain data.
 *
 * `close` drops the cached SQLite handles without tearing the worker down — used
 * when a region pack is added/removed so the next query reopens against the new
 * set of files.
 */
/** What to do — the `id` is added by the client for correlation (see below). */
export type GeocodeWorkerCall =
  | { op: "forward"; req: GeocodeForwardRequest; ep: Endpoint }
  | { op: "reverse"; req: GeocodeReverseRequest; ep: Endpoint }
  | { op: "close" };

export type GeocodeWorkerRequest = GeocodeWorkerCall & { id: number };

export type GeocodeWorkerResponse =
  | { id: number; ok: true; results: GeocodeResult[] }
  | { id: number; ok: false; error: string };

/** Run one request. Throws on a malformed op; the worker turns that into an error reply. */
export function handleGeocodeRequest(msg: GeocodeWorkerRequest): GeocodeResult[] {
  switch (msg.op) {
    case "forward":
      return forwardQuery(msg.req, msg.ep);
    case "reverse":
      return reverseQuery(msg.req, msg.ep);
    case "close":
